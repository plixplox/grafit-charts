...
| `marker.showOn`      | `'always' \| 'hover'`                    | `'always'`                        | `'hover'` — только у подсвеченной точки                                                                    |
...
| `marker.itemStyler`  | `(params) => style`                      | —                                 | стиль одного маркера по `datum` (fill, stroke, strokeWidth, size, label.color); линия остаётся цвета серии |
