...
| `marker.enabled` (area)    | `boolean`                              | маркеры на обоих краях (выкл.; `showOn: 'hover'` — на подсвеченном)    |
| `marker.itemStyler` (area) | `(params) => style`                    | стиль одного маркера; в params ещё `edge`, `low`, `high`               |
