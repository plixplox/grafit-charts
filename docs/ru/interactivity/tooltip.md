# Тултип

Включён по умолчанию: при наведении показывается ближайший узел, сам узел подсвечивается, остальные серии приглушаются.

::: tip Модульная сборка
В сборке через [grafit-charts/core](/ru/guide/bundle) тултип — отдельный модуль: `register(tooltipModule)`.
:::

По умолчанию тултип показывает значение `xField` заголовком и пару «имя серии: значение» с маркером серии. Исключение — точечные серии (scatter, bubble): у них обе оси — измерения, поэтому заголовок обозначает серию (маркер + `name`), а значения выводятся подписанными строками — `xName: x`, `yName: y`, у bubble ещё `sizeName: size`. Содержимое настраивается через `tooltip.renderer` на серии:

::: chart-example tooltip-custom

## Режимы

`mode: 'single'` (по умолчанию) показывает значение ближайшего узла;
`mode: 'shared'` — значения всех видимых серий категории одним тултипом; подсвечиваются узлы всех серий, приглушение не применяется:

::: chart-example tooltip-shared

Режим означает одно и то же везде, где серии делят категорию: радар несёт по
мере на серию так же, как линейный график, поэтому спица под курсором отвечает
сразу за все, — и так же ведут себя роза и радиальные столбцы, у которых общая
угловая ось.

::: chart-example tooltip-shared-radar

## Опции (chart-level)

| Опция               | Тип                                | По умолчанию               | Описание                                                                                                                  |
| ------------------- | ---------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `enabled`           | `boolean`                          | `true`                     | показывать тултип                                                                                                         |
| `mode`              | `'single' \| 'shared'`             | `'single'`                 | один узел или вся категория                                                                                               |
| `position.anchorTo` | `'node' \| 'center' \| 'pointer'`  | `'node'`                   | край узла, центр узла или курсор                                                                                          |
| `position.xOffset`  | `Pixels`                           | `0`                        | смещение тултипа                                                                                                          |
| `yOffset`           | `Pixels`                           | `0`                        | смещение тултипа                                                                                                          |
| `range`             | `Pixels \| 'exact' \| 'nearest'`   | `30`                       | число — радиус в px; `'exact'` — только попадание в узел; `'nearest'` — ближайший узел из любого места области построения |
| `container`         | `'chart' \| 'body' \| HTMLElement` | `'chart'`                  | где живёт элемент тултипа — см. [Вне графика](#вне-графика)                                                               |
| `zIndex`            | `number`                           | `10`; вне графика — `1000` | CSS z-index элемента тултипа                                                                                              |

## Вне графика

Тултип — DOM-элемент внутри контейнера графика, прижатый к его границам. В
плитке дашборда с `overflow: hidden` или на графике высотой в несколько десятков
пикселей места ему не хватает: тултип обрезается или закрывает сам график.
`container: 'body'` переносит его в `document.body` с `position: fixed` — он
ставится у узла в координатах окна и прижимается к границам окна, поэтому
обрезающие предки его больше не режут:

```ts
Charts.create({
  container,
  tooltip: { container: 'body' },
  series: [{ type: 'area', xField: 'day', yField: 'visits', marker: { showOn: 'hover' } }],
});
```

Полоса KPI высотой 48 пикселей в рамке, которая обрезает содержимое, как карточка
дашборда. Наведите на неё курсор, а потом кнопкой ниже переключите тултип внутрь
графика и сравните:

::: chart-example tooltip-outside 48

Элемент вместо `'body'` кладёт тултип в этот элемент — для приложений, которые
держат оверлеи в своём слое; позиционируется он так же. Вне графика:

- если точка ближе к верхнему или нижнему краю графика, чем высота тултипа,
  тултип встаёт сразу за этим краем — над графиком, а если сверху нет места, под
  ним — вместо того чтобы лечь поверх графика; дальше от краёв тултип остаётся у
  точки. На полосе KPI или спарклайне все точки у края, поэтому график тултип не
  закрывает;
- прокрутка страницы или любого прокручиваемого предка и изменение размера окна
  прячут тултип — следующее движение курсора покажет его снова на нужном месте;
- `destroy()` убирает элемент вместе с графиком;
- шрифт, размер, насыщенность и цвета темы задаются на самом элементе: от
  контейнера графика он ничего не наследует;
- `position: fixed` отсчитывается от окна, если у предков элемента нет
  `transform`, `filter` или `contain` — не ставьте их на переданный слой.

## Внешний вид

Контейнер тултипа настраивается там же, в `tooltip`:

| Опция          | Тип                      | По умолчанию                | Описание                                                                                        |
| -------------- | ------------------------ | --------------------------- | ----------------------------------------------------------------------------------------------- |
| `background`   | `ColorValue`             | фон темы                    | фон                                                                                             |
| `borderColor`  | `ColorValue`             | приглушённый цвет темы      | цвет рамки                                                                                      |
| `borderWidth`  | `Pixels`                 | `1`                         | толщина рамки; `0` убирает рамку                                                                |
| `borderRadius` | `Pixels`                 | `6`                         | скругление углов                                                                                |
| `shadow`       | `string \| false`        | `0 2px 8px rgba(0,0,0,.25)` | CSS box-shadow; `false` — без тени                                                              |
| `padding`      | `PaddingValue \| string` | `7px 10px`                  | внутренние поля: `8`, `[8, 12]`, `[8, 12, 4, 0]`, `{ top, right, bottom, left }` или CSS-строка |
| `fontSize`     | `Pixels`                 | `12`                        | размер шрифта                                                                                   |
| `fontFamily`   | `string`                 | `system-ui, sans-serif`     | гарнитура                                                                                       |
| `color`        | `ColorValue`             | foreground темы             | цвет текста                                                                                     |

```ts
tooltip: {
  background: '#141821',
  color: '#f0f0f0',
  borderColor: '#436ff4',
  borderRadius: 10,
  shadow: false,
},
```

## Опции серии

`series[].tooltip.renderer(params)` возвращает строку (станет заголовком) или структуру:

```ts
renderer: ({ datum, xValue, yValue, seriesName, color }) => ({
  heading: String(xValue),
  rows: [{ label: seriesName, value: String(yValue), color }],
});
```

`params.datum` — вся строка данных: в тултип можно выводить поля, не участвующие в серии.

`heading` — строка либо объект `{ text, color }`; с `color` перед текстом заголовка рисуется такой же маркер, как у строк. Именно так scatter и bubble рисуют тултип по умолчанию — маркер обозначает серию, а не отдельную строку:

```ts
renderer: ({ datum, xValue, yValue, seriesName, color }) => ({
  heading: { text: String(datum.category), color },
  rows: [
    { label: 'X', value: String(xValue) },
    { label: 'Y', value: String(yValue) },
  ],
});
```

Подсветка управляется блоком `highlight`:

| Опция                  | Тип        | По умолчанию | Описание                                              |
| ---------------------- | ---------- | ------------ | ----------------------------------------------------- |
| `enabled`              | `boolean`  | `true`       | подсветка узла и приглушение остальных серий          |
| `dimOpacity`           | `Fraction` | `0.8`        | прозрачность неподсвеченных серий (1 — не приглушать) |
| `highlight.enabled`    | `boolean`  | `true`       | подсветка узла и приглушение                          |
| `highlight.dimOpacity` | `Fraction` | `0.8`        | прозрачность неподсвеченных серий                     |
