...
- рамка (`boxSelect: true`) работает в `multiple` везде, где у элемента есть место на
  экране: декартовы серии (line, bar, area, scatter/bubble), вершины радара, секторы розы
  и радиальных столбцов, плитки treemap, sunburst и потоковых серий. Pie и donut, а также
  стадийные серии (funnel, cone-funnel, pyramid) выбираются кликами — выбранный сектор,
  стадия или слой получает обводку, остальные приглушаются;
- полярный и иерархический элемент ловится той точкой, которой чарт его адресует:
  вершина радара, середина сектора, прямоугольник плитки, — поэтому рамка, задевшая
  сектор, но не накрывшая его середину, его не выберет;
