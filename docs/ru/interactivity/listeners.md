...
Иерархия нумерует все узлы, которые рисует, включая вложенные, поэтому у treemap и
sunburst `datumIndex` считает узлы, а не строки `data`. При этом `datum` — по-прежнему
та строка, из которой прочитан узел: для вложенного это объект из `children`, что и
нужно для drill-down по иерархии.
...
- `datum` — ссылка на исходный объект из `data` (не копия), включая вложенные
  строки `children` у иерархических серий.
