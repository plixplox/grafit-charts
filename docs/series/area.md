...
`marker.showOn: 'hover'` keeps the area bare and draws a marker only on the
highlighted point — by the pointer, the keyboard or a shared tooltip. Handy for
small charts where a marker on every point is noise. Works the same on a line:
...
| `marker.showOn`      | `'always' \| 'hover'`                    | `'always'`                            | `'hover'` — only the highlighted point; turns on markers by itself                                                 |
...
| `marker.itemStyler`  | `(params) => style`                      | —                                     | style of one marker by its `datum` (fill, stroke, strokeWidth, size, label.color); the area keeps the series color |
