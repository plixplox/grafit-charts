...
| `marker.showOn`      | `'always' \| 'hover'`                    | `'always'`                         | `'hover'` — only the highlighted point                                                                             |
...
| `marker.itemStyler`  | `(params) => style`                      | —                                  | style of one marker by its `datum` (fill, stroke, strokeWidth, size, label.color); the line keeps the series color |
