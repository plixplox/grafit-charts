...
| `marker.enabled` (area)    | `boolean`                              | markers on both edges (off; `showOn: 'hover'` — on the highlighted datum) |
| `marker.itemStyler` (area) | `(params) => style`                    | style of one marker; params add `edge`, `low`, `high`                     |
